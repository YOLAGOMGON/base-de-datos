...
const logger = require('./middlewares/logger'); // Importa el middleware de registro de peticiones.
...
app.use(logger); // Registra cada peticion en consola.
app.use('/usuarios', usuariosRoutes); // Monta las rutas de usuarios bajo /usuarios.
app.use((req, res) => { // Maneja rutas no encontradas.
  res.status(404).json({ error: 'Ruta no encontrada' }); // Responde con error 404 en JSON.
